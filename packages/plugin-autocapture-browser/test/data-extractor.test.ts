...
describe('data extractor', () => {
  ...
  describe('updateSelectorConfig', () => {
    ...
    test('a null delivery does not disable a previously-enabled engine', () => {
      document.getElementsByTagName('body')[0].innerHTML = `
        <section><span class="lbl">a</span><span class="lbl">b</span></section>
      `;
      const target = document.querySelectorAll('span')[1];

      dataExtractor.updateSelectorConfig({ enabled: true });
      // New engine: positional :nth-of-type, no classes.
      expect(dataExtractor.getElementPath(target)).toContain(':nth-of-type');

      // A null delivery (failed/empty remote fetch) must NOT revert to legacy.
      dataExtractor.updateSelectorConfig(null);
      const afterNull = dataExtractor.getElementPath(target);
      expect(afterNull).toContain(':nth-of-type');
      expect(afterNull).not.toContain(':nth-child');
    });

    test('an explicit enabled:false reverts to the legacy cssPath', () => {
      document.getElementsByTagName('body')[0].innerHTML = `
        <section><span class="lbl">a</span><span class="lbl">b</span></section>
      `;
      const target = document.querySelectorAll('span')[1];
      ...
      // Explicit disable is honored (the kill switch still works).
      dataExtractor.updateSelectorConfig({ enabled: false });
      expect(dataExtractor.getElementPath(target)).toContain(':nth-child');
    });

    test('an incomplete payload (no boolean enabled) does not disable a live engine', () => {
      document.getElementsByTagName('body')[0].innerHTML = `
        <section><span class="lbl">a</span><span class="lbl">b</span></section>
      `;
      const target = document.querySelectorAll('span')[1];
      ...
      // Partial delivery: patterns only, no `enabled` flag. Must be ignored,
      // not reset to dormant defaults.
      dataExtractor.updateSelectorConfig({ autogeneratedIdPatterns: ['^x'] });
      const after = dataExtractor.getElementPath(target);
      expect(after).toContain(':nth-of-type');
