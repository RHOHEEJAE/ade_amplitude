...
export const matchEventToFilter = (event: ElementBasedTimestampedEvent<ElementBasedEvent>, filter: Filter) => {
  try {
    if (filter.subprop_key === 'element_text') {
      // TODO: add support for the other operators
      return (
        filter.subprop_op === 'is' &&
        filter.subprop_value.includes(event.targetElementProperties['element_text'] as string)
      );
    } else if (filter.subprop_key === 'element_hierarchy') {
